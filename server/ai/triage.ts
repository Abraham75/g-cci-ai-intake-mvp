...
function deriveScoreInput(payload: any): ScoreIntakeInput {
  ...
  const highSeverityUnresolved = contradictions.filter(
    (c: any) => c?.severity === "high" && c?.resolved !== true,
  ).length;
  const other = contradictions.length - highSeverityUnresolved;
  ...
  return {
    ...
    contradictions: { highSeverityUnresolved, other },
...
export function triageIntake(payload: any) {
  ...
  const scoreInput = deriveScoreInput(payload);
  const scoring = scoreIntake(scoreInput);

  const scoreEntry = appendLedgerEntry({
    entryType: "Score",
    subjectId: intakeId,
    payload: { input: scoreInput, result: scoring },
    producedBy: "ai.scoreIntake",
    inputEntryIds: [evidenceEntry.id, temporalEntry.id],
